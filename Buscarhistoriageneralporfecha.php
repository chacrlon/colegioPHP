...
<br><br><br>
   ...
   <div class="container">
    ...
    if($anio!=""){
        if($mes==0){
            $sql5 = "SELECT * FROM historia WHERE EXTRACT(YEAR FROM historia.fecha)='$anio' ORDER BY historia.id_historia DESC ";
                   
        }
        else{
            if($dia!=""){
                $sql5 = "SELECT * FROM historia WHERE EXTRACT(YEAR FROM historia.fecha)='$anio' AND EXTRACT(MONTH FROM historia.fecha)='$mes' AND EXTRACT(DAY FROM historia.fecha)='$dia' ORDER BY historia.id_historia DESC ";
              
            }
            else{
                $sql5 = "SELECT * FROM historia WHERE EXTRACT(YEAR FROM historia.fecha)='$anio' AND EXTRACT(MONTH FROM historia.fecha)='$mes' ORDER BY historia.id_historia DESC ";
            }
        }
    }
...
<table align="right">
                            <tr>
                                <td >
                                    <br>
                                    <form  action="Estudiante2.php" method="POST" "> 
    ...
    ';
            ...
            if($resultado5 = mysql_query($sql5)){
                if(mysql_num_rows($resultado5)==0){
                     echo "<script language='javascript'>alert('No hay Acontecimientos Registrados para está Fecha');</script>";
                ...
                <br>
                         <div class="panel-heading fondorojo bordeazul letrablanca text-center">
                                 <h4> <strong>ACONTECIMIENTOS</strong></h4>
                            </div>
                     <br>';
